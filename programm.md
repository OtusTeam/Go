...
Завести Go репозиторий на GitHub, написать программу печтающую текущее время / точное время с использованием библиотеки NTP
...
Создать Go функцию, осуществляющую примитивное сжатие строки, содержащей повторяющиеся сиволы / руны, например: "aaaabccddddde"  => "a4bc2d5e"
...
Написать функцию, которая получае на вход текст и возвращает
...
Примает на вход слайс с заданиями `[]func()error`, число заданий которые можно выполнять параллельно `N` и максимальное число ошибок после которого нужно приостановить обработку. Учесть что задания могу выполняться разное время.
...
Описать и проверить установку зависимостей с помощью gep.
...
Эта утилита позволяет запускать пргограммы получая переменные окружения из определенной директории. См man envdir
