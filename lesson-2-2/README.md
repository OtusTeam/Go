...
* `bufio` - буфферизованный ввод / вывод
* `fmt` - форматированный вывод и парсинг
...
buf := make([]byte, N)  // подготавливаем буффер нужного размера
...
Заметим, что тип `os.File` реализует интефейс `io.Reader`:
...
<br><br>
Метод `Read` читает данные (из объекта) в буффер `p`, не более чем `len(p)` байт.
<br><br>
...
Гарантированно заполнить буффер
```
...
* `io.SeekEnd` - относильно конца файла
...
Используя методы `Read`, `Write` и промежуточный буффер не сложно сделать копирование между двумя файловыми (и не только).

<br><br>
А можно использовать и готовые реалиазации:
...
// копирует все вплоть до io.EOF, но использует заданный буффер
...
то копирование с помощью `io.Copy` может использовать оптимизацию и НЕ выделять промежуточный буффер.
...
# Буфферизация ввода/вывода

С помощью пакета `bufio` можно сократить число системных вызовов и улучшить производительность в случае
если требуется читать/записывть данные небольшими кусками, например по строкам.
<br><br>
Запись:
```
file, _ := os.Create(path)
...
bw.WriteRune('±')
bw.WriteByte(42)
bw.Flush()   // очистить буффер, записать все в file
```
...
Общие:
```
%v  - дефолтное представление для типа
...
Также можно управлять рассширенным представлением (`%#v`), реализовав `GoStringer`
```
...
Для упрощения парсинга командной строки можно использовать пакет `flag`
...
func main() {
  flag.Parse() // распарсить аргументы
